					...
					<form method="post">
					 <div class="box-body">
					 	<?php
							...
							{
								$sc1=sprintf("Select * from user where id_user='%s' and password='%s'",$_SESSION['id_user'],($_POST['old_password']));
								
								$q1=mysql_query($sc1);
								$rc1=mysql_num_rows($q1);
								if($rc1==1)
								{
									$sc2=sprintf("Update user Set password='%s' Where id_user='%s'",($_POST['new_password']),$_SESSION['id_user']);
									$q2=mysql_query($sc2);
									if($q2)
									{
